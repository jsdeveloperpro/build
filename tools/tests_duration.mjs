...
const measureDuration = async () => {
  ...
  const testFiles = await globby(packageJson.ava.files)
  ...
  for (const testFile of testFiles) {
    ...
    await execa('ava', [testFile], { preferLocal: true, reject: false })
...
const getOrder = ({ filesMachines }) => {
  ...
  const order = [...filesMachines.entries()]
    ...
    .map(([file, { machine, duration }], index) => ({ file, order: index, machine, duration }))
...
const main = async () => {
  ...
  await fs.writeFile('tests-duration.json', `${JSON.stringify(order, null, 2)}\n`)
}
